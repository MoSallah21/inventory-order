...
`product-image.ts` is the bounded trust boundary for browser `File` values: it checks 5 MiB before reading, permits only
JPEG/PNG/WebP, verifies MIME/signature agreement, and performs bounded baseline/progressive JPEG scan-marker, PNG
chunk, or WebP RIFF/chunk validation. WebP supports still `VP8 `, `VP8L`, and `VP8X` containers with actual image
payload; animation is rejected. The strict extended subset requires `VP8X`, optional pre-image `ICCP`, optional lossy
`ALPH` immediately adjacent to `VP8 `, one reconstructive payload, then optional `EXIF` and `XMP ` in either order.
Flags must match their chunks or intrinsic VP8L alpha bit; duplicates, unknown chunks, and reconstructive data after the
payload are rejected. This is structural validation, not full decoding or PNG CRC verification. The Server Action multipart
envelope is 6 MiB so one file at the application maximum plus bounded text/boundary overhead reaches this validator.
`image-storage.ts` is server-only and hides Cloudinary SDK calls, credential access, controlled key generation, strict
delivery URL/format/byte/dimension response validation, and namespace-restricted cleanup from the catalog.

Creation uploads before the database insert and compensates an insert failure by deleting the new object. Replacement
captures trusted image/archive state before upload, then locks Product before Category and compares current state inside
the transaction. Stale replacement/removal fails with `CONFLICT`; a losing upload is compensated and users may retry.
The winner commits the new trusted URL/key before best-effort deletion of the exact previous managed object; failed post-commit cleanup
returns a safe warning and never rolls back the usable image. Removal clears both non-null string columns atomically
with empty strings, then deletes only keys in the controlled namespace. Legacy external URLs/keys are cleared but never
sent to Cloudinary. Archive retains image metadata and objects.

Provider evidence must contain the exact generated public ID, mandatory positive dimensions, bounded bytes, matching
format, and an exact-object Cloudinary delivery path with no transformation/query/fragment ambiguity. Persistence uses
a canonical URL rebuilt from trusted cloud name, expected key, and normalized format—not the returned URL.
Provider-response validation failure triggers best-effort deletion of only the expected generated key. A distinct
untrusted returned key is never a deletion authority. Cleanup failures emit only operation, safe expected key, and
sanitized error category to server stderr; the original validation/database error remains authoritative.

Parser fixtures are anonymous 3×2 images generated by Pillow 11.3.0 and independently decoded through macOS ImageIO
(`sips`) and identified with `file`; provenance is recorded under `tests/fixtures`. Action-order integration tests mock
only session transport and Cloudinary while exercising real PostgreSQL actor reload, role/disabled checks, ownership,
parser ordering, and actor-derived supplier persistence.

The supported VP8X subset rejects its animation flag and every `ANIM`/`ANMF` chunk, requires reserved bits/bytes to be
zero, checks ICC/alpha/EXIF/XMP feature consistency, and requires the canvas to equal the single still VP8/VP8L payload.
Cloudinary evidence accepts one optional lowercase `v` segment containing 1–20 decimal digits. Adversarial coverage
includes separator encoding/double encoding, backslashes, suffixes/extensions, version confusion, transformations,
extra paths, queries, fragments, credentials, and ports.

Action integration tests separately prove cross-owner replacement and removal denial before parsing/provider access.
An owning-supplier replacement injects likely previous-key/URL/public-ID/supplier fields and proves the previous delete
key is loaded from PostgreSQL while new metadata comes only from validated provider output. Every image race production
operation is immediately tracked and awaited through a named 3-second result timeout; cleanup releases gates, aborts
uploads, and uses a named 5-second bounded settlement.

Cloudinary and PostgreSQL cannot share a transaction. Compensation and post-commit cleanup minimize, but cannot fully
eliminate, orphaned objects during process/network failure. Pagination, notifications, exports, caching, and rate
